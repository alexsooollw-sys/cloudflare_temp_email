...
- feat: |Worker| Tempmail runtime config (`tempmail.allowed_domains`, `accounts_per_day_per_ip`, `account_ttl_hours`, `rpm`, `rps`, `max_attachment_mb`, `max_body_mb`, `enable_public_preview`, …) lives in the new `system_settings` table with conservative defaults — tunable through admin UI once Stage 2 admin Vuetify port lands
- feat: |Mail UI| `@cte/mail` package — a notletters-style centered login card + inbox + password-change view + theme & locale switcher, hitting the existing `/api/address_login`, `/api/parsed_mails`, `/api/parsed_mail/:id`, `/api/address_change_password` endpoints
- feat: |Tempmail UI| `@cte/tempmail` package — a mail.tm-style landing page + create / login dialogs + inbox with auto-refresh + optional public preview, all wired to `/public_api/v1/*`. Ships with EN / RU bundled i18n
- feat: |Docs| New `DEPLOY.md` at the repository root plus `guide/multi-site-setup.md` and `guide/feature/public-api.md` in `vitepress-docs` (EN + ZH) covering DNS / Email Routing / three Pages projects / DB migrations / initial admin bootstrap
